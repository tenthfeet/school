...
const validator = validatorInit('#department-form', {
    rules: {
...
function submitForm(form, event) {
    ...
    let id = data.get('id');
    let isUpdate = !!id;
    let url = isUpdate ? `/departments/${id}` : '/departments';
    let method = 'POST';
    if (isUpdate) {
...
function resetForm() {
    ...
    formCard.find('.card-header').html('Add new Department');
    formBtn.html('Submit');
}
...
const showUpdateForm = async function (element) {
    ...
    formCard.find('.card-header').html('Update Department');
    formBtn.html('Update');
    window.scrollTo(0, 0);
};
