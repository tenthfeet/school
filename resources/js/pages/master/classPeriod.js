...
const form = $('#class-period-form');
const formCard = form.closest('.card');
const formBtn = form.find('button[type="submit"]');

const validator = validatorInit('#class-period-form', {
    rules: {
    ...
    messages: {
        name: {
            required: 'Please enter the period name',
        },
        start_time: {
...
function submitForm(form, event) {
    ...
    let id = data.get('id');
    let isUpdate = !!id;
    let url = isUpdate ? `/class-periods/${id}` : '/class-periods';
    let method = 'POST';
    if (isUpdate) {
...
function resetForm() {
    ...
    formCard.find('.card-header').html('Add new Class Period');
    formBtn.html('Submit');
}
...
const showUpdateForm = async function (element) {
    ...
    const { data } = await axios.get(`/class-periods/${id}`);
    let fields = ['id', 'name', 'start_time', 'end_time', 'is_active'];
    fields.forEach(field => {
        form.find(`[name="${field}"]`).val(data[field]);
    });
    formCard.find('.card-header').html('Update Class Period');
    formBtn.html('Update');
    window.scrollTo(0, 0);
};
