...
const form = $('#academic-standard-form');
const formCard = form.closest('.card');
const formBtn = form.find('button[type="submit"]');

const validator = validatorInit('#academic-standard-form', {
    rules: {
    ...
    messages: {
        name: {
            required: 'Please enter the Standard name',
        },
    },
...
function submitForm(form, event) {
    ...
    let id = data.get('id');
    let isUpdate = !!id;
    let url = isUpdate ? `/academic-standards/${id}` : '/academic-standards';
    let method = 'POST';
    if (isUpdate) {
...
function resetForm() {
    ...
    formCard.find('.card-header').html('Add new Academic Standard');
    formBtn.html('Submit');
}
...
const showUpdateForm = async function (element) {
    ...
    const { data } = await axios.get(`/academic-standards/${id}`);
    let fields = ['id', 'name', 'is_active'];
    fields.forEach(field => {
        form.find(`[name="${field}"]`).val(data[field]);
    });
    formCard.find('.card-header').html('Update Academic Standard');
    formBtn.html('Update');
};
