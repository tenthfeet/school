...
const form = $('#medium-of-study-form');
const formCard = form.closest('.card');
const formBtn = form.find('button[type="submit"]');

const validator = validatorInit('#medium-of-study-form', {
    rules: {
    ...
    messages: {
        name: {
            required: 'Please enter the medium name',
        },
    },
...
function submitForm(form, event) {
    ...
    let id = data.get('id');
    let isUpdate = !!id;
    let url = isUpdate ? `/medium-of-studies/${id}` : '/medium-of-studies';
    let method = 'POST';
    if (isUpdate) {
...
function resetForm() {
    ...
    formCard.find('.card-header').html('Add new Medium');
    formBtn.html('Submit');
}
...
const showUpdateForm = async function (element) {
    ...
    const { data } = await axios.get(`/medium-of-studies/${id}`);
    let fields = ['id', 'name', 'is_active'];
    fields.forEach(field => {
        form.find(`[name="${field}"]`).val(data[field]);
    });
    formCard.find('.card-header').html('Update Medium');
    formBtn.html('Update');
};
